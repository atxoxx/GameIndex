...
const TABS: {
  ...
  {
    id: "coming_soon",
    labelKey: "store.featured.comingSoon",
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
export default function StoreFeaturedHero({ onPickGame }: StoreFeaturedHeroProps) {
  ...
  const [aboutBundle, setAboutBundle] = useState<AboutBundle | null>(null);
  ...
  // Reset per-game states when activeGame changes and fetch details & about bundle
  useEffect(() => {
    setLogoFailed(false);
    ...
    const steamId = resolveSteamAppId(activeGame);
  ...
  }, [activeGame?.id, activeGame?.slug, activeGame?.name]);
  ...
  const descriptiveText = useMemo(() => {
    if (!activeGame) return "";
    if (aboutBundle?.byLanguage) {
      const targetSteamLang = steamCodeForUi(language);
      const currentLangPayload = aboutBundle.byLanguage[targetSteamLang];
      const localizedText =
        currentLangPayload?.shortDescription?.trim() || currentLangPayload?.aboutText?.trim();
      if (localizedText) return localizedText;

      const englishPayload =
        aboutBundle.byLanguage["english"] ??
        aboutBundle.byLanguage[aboutBundle.defaultLanguage];
      const englishText =
        englishPayload?.shortDescription?.trim() || englishPayload?.aboutText?.trim();
      if (englishText) return englishText;
    }
    return activeGame.summary || metadata?.description || metadata?.storyline || "";
  }, [activeGame, aboutBundle, language, metadata]);

  const releaseYear = useMemo(() => {
    if (!activeGame?.firstReleaseDate) return null;
    const y = new Date(activeGame.firstReleaseDate).getFullYear();
    return isNaN(y) ? null : y;
  }, [activeGame?.firstReleaseDate]);

  const releaseInfo = useMemo(() => {
    if (!activeGame) return null;
    const targetSteamLang = steamCodeForUi(language);
    const steamReleaseDate =
      aboutBundle?.byLanguage[targetSteamLang]?.releaseDate ||
      aboutBundle?.byLanguage["english"]?.releaseDate ||
      aboutBundle?.byLanguage[aboutBundle?.defaultLanguage || ""]?.releaseDate;
    const platformDate =
      metadata?.releases?.find((r) =>
        r.platform.toLowerCase().includes("pc") || r.platform.toLowerCase().includes("win")
      )?.dateStr || metadata?.releases?.[0]?.dateStr;

    const rawDate =
      activeGame.firstReleaseDate || metadata?.releaseDate || steamReleaseDate || platformDate;
    if (!rawDate) return null;

    const trimmed = rawDate.trim();
    if (/^\d{4}$/.test(trimmed)) {
      const yearNum = parseInt(trimmed, 10);
      return { display: String(yearNum), isUpcoming: yearNum > new Date().getFullYear() };
    }
    ...
    if (!isNaN(d.getTime())) {
      ...
      try {
        dateString = d.toLocaleDateString(locale, {
          year: "numeric",
          month: "short",
          day: "numeric",
      ...
      return { display: dateString, isUpcoming };
    }

    return { display: trimmed, isUpcoming: true };
  }, [activeGame, metadata, aboutBundle, language]);
        ...
        >
          ...
          <div className="store-spotlight-body">
            <div className="store-spotlight-badge-cluster">
              ...
              {releaseInfo ? (
                <span
                  className={`store-spotlight-release${releaseInfo.isUpcoming ? " store-spotlight-release--upcoming" : ""}`}
                  title={releaseInfo.display}
                >
