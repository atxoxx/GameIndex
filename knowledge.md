...
- **Linux WebKitGTK rendering workarounds are NVIDIA+Wayland-gated** (`src-tauri/src/main.rs`): `WEBKIT_DISABLE_DMABUF_RENDERER` + `__NV_DISABLE_EXPLICIT_SYNC` are auto-set only when `gpu_detector::linux_has_nvidia_gpu()` detects a `0x10de` vendor under `/sys/class/drm` AND the session is Wayland (`WAYLAND_DISPLAY` or `XDG_SESSION_TYPE=wayland`). They fix the NVIDIA black-webview + torn-frames bugs while keeping the GL compositor, so navigation stays GPU-accelerated. `WEBKIT_DISABLE_COMPOSITING_MODE` / `__GL_THREADED_OPTIMIZATIONS` are NO LONGER auto-set — the former forces single-threaded software painting (the classic "hangs while navigating" on NVIDIA+Wayland) and its 2.44-era deadlock workaround is upstream-fixed; users who still need them must export the env vars themselves (a user-exported env var always wins). Never apply the auto set on AMD/Intel or NVIDIA/X11 — those keep full hardware acceleration. `GDK_BACKEND=wayland,x11` stays unconditional. The frontend mirrors this with `data-platform` on `<html>` (set by `SettingsContext`) + `styles/platform-linux.css`, which drops `backdrop-filter` from full-window/full-screen/sticky/large surfaces on Linux — WebKitGTK renders backdrop blur far slower than WebView2/Chromium (small pills/badges keep their blur).
