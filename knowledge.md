...
- **Updater:** `tauri-plugin-updater` for release builds, plus `updater.rs` for the portable Windows path. `updater_install_info` classifies the running build (`nsis`, `appimage`, `deb`, `portable`, `dev`, `unsupported`) and `UpdateContext` offers only the artifact matching this install — AppImage/`.deb` updates are plugin-managed, portable uses custom download → signature verify → `apply.cmd`. Manifest platform keys: `windows-x86_64` (portable), `windows-x86_64-nsis`, `linux-x86_64` (AppImage), `linux-x86_64-deb`.
- **Release history:** the updater section's "Release History" button opens `ChangelogModal` (`src/components/ui/ChangelogModal.tsx` + `src/utils/releaseNotes.ts`), which fetches `https://api.github.com/repos/atxoxx/GameIndex/releases?per_page=100` (direct fetch first, Rust `fetch_url` fallback under Tauri), filters drafts, parses Markdown bodies, and badges Latest / Pre-release / Installed releases.
- **Launch splash:** the splash renders in its own always-on-top frameless `launch-splash` window (`src-tauri/src/launch_splash.rs`), so it survives minimize-on-launch hiding the main window. `SplashContext.open()` invokes `open_launch_splash` **before** `launch_game`; `LaunchSplashBridge` mirrors record status changes and closes it on exit. `open_launch_splash` must stay `async` — a sync version deadlocks WebView2 on Windows (wry#583: window creation inside `WebMessageReceived`). The inline `<Splashscreen />` inside `SplashProvider` (z-index 9500) is the fallback when window creation fails.
...
- **Layout:** `App.tsx` wraps `ThemeProvider > LanguageProvider > ToastProvider > UpdateProvider > SplashProvider > GameProvider > ActivityProvider > AchievementProvider > DensityProvider > LibraryFilterProvider > WishlistProvider > SourceProvider > DownloadProvider > SettingsProvider > SessionNotesProvider > SteamGridDbProvider > CrackWatchProvider > PriceProvider > BigScreenProvider > PresenceProvider`. The shell renders `TopNav`, `Sidebar`, and `MainContent` via nested routes. The splash normally lives in its own `launch-splash` window (see "Launch splash" above); the inline `<Splashscreen />` inside `SplashProvider` at z-index 9500 is only the fallback. `BigScreenLayout` is lazy-loaded (heavy controller shell). `AdaptiveThemeSync` + `GameAccentSync` mount inside `PresenceProvider`; `SidebarCollapseProvider` wraps the shell and `GamepadProvider` is enabled only while Big Screen is active.
- **Pages (`src/pages/`)** — desktop pages, each its own lazy chunk:
  ...
  - `GamePage` — rich detail view with hero, metadata, reviews, achievements, screenshots, web links (native webview preview), live player count, force-close, a per-game **Notes** workspace (`src/components/game/notes/`), and (on Linux) a Wine-log viewer plus a per-game Proton/Wine tab inside `EditGameModal`.
  ...
  - `DocsPage` (`/docs`) — in-app user guide, content fully in i18n `docs.*` keys. The section model + markdown-ish parser live in `src/components/docs/docsContent.tsx` and are shared with `BigScreenDocsPage`; add or reorder a section in `DOC_SECTIONS` (desktop and Big Screen both follow it).
  - Default redirect on `/` → `LandingRedirect` (`/library`).
- **Components (`src/components/`)** — grouped by area: `game/` (incl. `EditGameModal`, `WineLogsModal`, `SteamFeaturesCard`, `GameRelationsCard`, `BigScreenHeroBackground`, `notes/`), `library/`, `store/` (incl. `StoreCompareModal` / `StoreCompareTray` + `storeCompare.ts`, `BigScreenStore`), `downloads/`, `download-modal/`, `news/`, `activity/`, `reviews/` (BBCode parser + review rows), `filters/`, `sidebar/`, `charts/`, `bigscreen/`, `friends/`, `deals/`, `command-palette/`, `hero/`, `weblinks/`, `mods/`, `emulators/`, `achievements/`, `ui/` (`Card`, `Button`, `Badge`, `KpiTile`, `MultiSelectDropdown`, `TagInput`, `Skeleton`, `Tooltip`, `ConfirmModal`, `PageHeader`, `UpdateModal`, `ChangelogModal`), plus top-level `SteamStatsPopoverShell` used by the player-count popovers.
...
> Full user-facing feature guide: [`docs/linux-wine-proton.md`](./docs/linux-wine-proton.md). This section is the implementation map.
...
- **System diagnostics:** `get_compatibility_system_status` probes OS/kernel, display server, Vulkan, GameMode, MangoHud, Gamescope, Winetricks, UMU and the EAC/BattlEye runtimes; the same tab's Runner Maintenance kills Wine/Proton processes and launches winecfg/winetricks/regedit/taskmgr.
- **UI/gating:** global `CompatibilityTab.tsx` (subtabs runners, prefixes, graphics, sync engine, gamescope, tools, env/DLL, diagnostics) and the per-game `EditGameModal` "compatibility" tab render only when `showFullLinuxUi`; `LinuxSupportLevel = disabled | deck_verified | full` lives in `SettingsContext` (localStorage `gamelib.linux_support_level`). Rust `#[cfg(target_os = "linux")]` blocks cover Steam-via-compat routing plus MangoHud/GameScope; umu resolution returns `None` off-Linux and the rest compiles cross-platform.
...
### Game Notes
- Per-game notes workspace on the game page (`src/components/game/notes/NotesTab.tsx`, `NoteEditor.tsx`, `NoteListItem.tsx`, local `markdown.tsx` renderer; styles in `src/styles/game-notes.css`). Notes are Markdown, support tags, pinning, search/tag filters, copy/export and debounced auto-save (flushed on blur, note switch, unmount and Ctrl/Cmd+S).
- Persistence: the `game_notes` SQLite domain (`db/game_notes.rs`, commands `load_game_notes` / `save_game_note` / `delete_game_note` / `delete_game_notes_for_game`), with a `localStorage` fallback (`gamelib-game-notes`) for frontend-only dev. `useGameNotes` owns the list and migrates the legacy single-string `game.notes` field into a real note exactly once.
- `src/utils/gameNotes.ts` holds the pure helpers (sorting, title derivation, word/char counts, export filename, Markdown formatting actions); the editor's toolbar covers headings, bold/italic/strike, inline code/code blocks, quotes, bullet/ordered/task lists and links.

### Backup & restore
- `backup.rs` exposes `backup_get_status`, `backup_inspect`, `backup_create`, `backup_restore` and emits `backup-progress`. The current archive is the raw **NDJSON v2 `.gibak`** format (`backup_raw.rs`, magic `gameindex-raw-backup`): one NDJSON file per domain plus `artwork/` and `plugins/`, with `replace` and `merge` restore modes. `BACKUP_DOMAINS` includes `compatibility` (settings, per-game profiles, installed runner records — config only; prefixes/logs on disk are not archived). `game_notes` is **not** in `BACKUP_DOMAINS` yet. Legacy v1 binary archives remain restorable. UI: `BackupTab.tsx` + `BackupProgressModal.tsx`.
...
- **Custom themes** — the Appearance tab hosts a theme creator with live preview, presets, and JSON import/export (`ThemeCreatorModal.tsx`, `ThemeImportModal.tsx`, `ThemePreviewPanel.tsx`); imported and created themes must honor the same CSS-variable contract as the built-ins.
...
- **Splash window creation:** `launch_splash::open_launch_splash` must stay `async fn` — `WebviewWindowBuilder::build` deadlocks WebView2 on Windows when called from a sync command/event handler (wry#583: creating a controller inside `WebMessageReceived` never completes), which previously hung the UI thread so `launch_game` enqueued right behind it never ran ("game doesn't launch on Windows", while Linux was fine). Every runtime window creator in this repo is async for this reason.
- **Game notes backup gap:** `game_notes` is not in `BACKUP_DOMAINS` — notes currently don't travel in `.gibak` backups. Add the domain to `backup.rs`/`backup_raw.rs` if notes must be restorable.
...
src-tauri/
  ...
  src/launch_splash.rs     Standalone launch-splash window (async-only; see gotchas)
...
docs/linux-wine-proton.md  Linux/Wine/Proton feature guide (user-facing)
