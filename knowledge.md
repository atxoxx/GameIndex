...
- **Linux WebKitGTK rendering workarounds are NVIDIA+Wayland-gated** (`src-tauri/src/main.rs`): `WEBKIT_DISABLE_DMABUF_RENDERER` / `WEBKIT_DISABLE_COMPOSITING_MODE` / `__NV_DISABLE_EXPLICIT_SYNC` / `__GL_THREADED_OPTIMIZATIONS` are only set when `gpu_detector::linux_has_nvidia_gpu()` detects a `0x10de` vendor under `/sys/class/drm` AND the session is Wayland (`WAYLAND_DISPLAY` or `XDG_SESSION_TYPE=wayland`). They fix NVIDIA black-screen/deadlock bugs but disable WebKitGTK hardware acceleration (software compositing → sluggish UI), so never apply them on AMD/Intel or on NVIDIA/X11 — all of those keep full GPU compositing. A user-exported env var always wins. `GDK_BACKEND=wayland,x11` stays unconditional. The frontend mirrors this with `data-platform` on `<html>` (set by `SettingsContext`) + `styles/platform-linux.css`, which drops `backdrop-filter` from full-window/full-screen surfaces on Linux — WebKitGTK renders backdrop blur far slower than WebView2/Chromium.
