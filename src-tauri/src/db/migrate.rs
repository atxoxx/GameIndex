...
mod tests {
    ...
    fn fresh_db_runs_v1_and_is_idempotent() {
        ...
        // After the v5 migration was added (games launch-orchestration
        // columns), a fresh DB ends up at "v5" — the assertion reflects
        // the current head. Re-running `run_migrations` is still a no-op
        // (the loop early-returns when `current == target`), so the test
        // stays idempotent.
